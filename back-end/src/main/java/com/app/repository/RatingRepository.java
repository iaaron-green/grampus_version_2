...
public interface RatingRepository extends JpaRepository<Rating, Long> {
    ...
    @Query("SELECT DISTINCT NEW com.app.DTO.DTOLikableProfile (u.id, u.fullName, u.jobTitle, p.profilePicture, count(r.ratingType)) " +
            " FROM Rating r JOIN Profile p ON p.id = r.profileRating.user.id JOIN User u ON u.id = p.id AND r.profileRating.user.id IN :userIds AND u.fullName LIKE :searchParam% OR u.jobTitle LIKE :searchParam% WHERE r.ratingType NOT LIKE :ratingType OR r.ratingType IS NULL" +
            " GROUP BY u.id")
    ...
    @Query("SELECT DISTINCT NEW com.app.DTO.DTOLikableProfile (u.id, u.fullName, u.jobTitle, p.profilePicture, count(r.ratingType)) " +
            " FROM Rating r JOIN Profile p ON p.id = r.profileRating.user.id JOIN User u ON u.id = p.id AND r.profileRating.user.id IN :userIds AND u.fullName LIKE :searchParam% OR u.jobTitle LIKE :searchParam% WHERE r.ratingType LIKE :ratingType" +
            " GROUP BY u.id")
    ...
    @Query("SELECT DISTINCT NEW com.app.DTO.DTOLikableProfile (u.id, u.fullName, u.jobTitle, p.profilePicture, count(r.ratingType)) " +
            " FROM Rating r JOIN Profile p ON p.id = r.profileRating.user.id JOIN User u ON u.id = p.id AND u.fullName LIKE :searchParam% OR u.jobTitle LIKE :searchParam% WHERE r.ratingType NOT LIKE :ratingType OR r.ratingType IS NULL" +
            " GROUP BY u.id")
