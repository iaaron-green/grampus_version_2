...
class PieChartViewController : UIViewController {
    ...
    func try1337() {
        ...
        if storage.getProfileState() {
            network.fetchAchievements(userId: storage.getUserId()!) { (achievements) in
                if let achievements = achievements {
                    pieVC.achievements = achievements
                }
            }
        } else {
            network.fetchAchievements(userId: storage.getSelectedUserIdProfile()!) { (achievements) in
                if let achievements = achievements {
                    pieVC.achievements = achievements
                }
            }
